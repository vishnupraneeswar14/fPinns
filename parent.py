...
class batch_norm_activation(nn.Module):
...
class Net(nn.Module):
    def __init__(self,layer_size : list[int],layer_count, freq) -> None:
        ...
        # activation = nn.Tanh()
        # activation = lambda x: (1-torch.exp(-(alphie*(x))**2))*nn.Tanh(x)
        activation = batch_norm_activation([nn.Tanh(), self.hidden_size, 0.6])
        # print("Init:")
        # print(activation)
...
freq=10
pinn=Net([1,32,1],3,freq)
m,k,c=2,200,2
xo ,vo =  0.5,0.

# dumm = torch.load("../batch_alpha_inverse_0.45.pth")
# pinn.load_state_dict(dumm["model_parameters"])
# a1 = torch.tensor([dumm["inverse_parameters"][0]], requires_grad=True)
# print("Parent alpha: ", a1)
...
T=20.
tau_actual=0.2*T
tau1= torch.tensor(torch.tensor([0.65*T]), requires_grad=True)
...
optimiser_tau   = torch.optim.Adam([tau1], lr=7.5*1e-2)
...
loss_prev = torch.tensor([0.], requires_grad=False)

prev_order= torch.tensor([0.], requires_grad=False)
count=0
...
for i in range(10000):
    ...
    if i % 39 == 0:
        print(f"Training step {i}, Time taken: {iteration_time:.4f}, alpha: {a1}, alpha_grad: {a1.grad}, tau: {tau1}, tau_grad: {tau1.grad}, Total log loss : {torch.log10(torch.tensor([loss]))}")
        ...
        plt.subplot(3,1,3)
        plt.plot(iters, torch.log10(torch.tensor([l])).view(-1))
        #   plt.hlines(tau_actual,0,len(tau_list), colors=['g'])
        ...
        plt.suptitle(f"Training step {i}")
        plt.savefig(f'Inverse_alpha_batch_norm_{a_actual}.jpg')
        # plt.savefig(f'Inverse_alpha_tau_batch_norm_{a_actual}_{tau_actual}.jpg')
    ...
    if i%78 == 0:
        imageio.mimsave(f'animation_Inverse_alpha_batch_norm_{a_actual}.gif', images, fps=10)
    if i%499 == 0:
        torch.save({"model_parameters" : pinn.state_dict(), "inverse_parameters" : a1.detach().cpu().numpy()}, f'batch_alpha_inverse_{a_actual}.pth')
    
    # if i%78 == 0:
    #     imageio.mimsave(f'animation_Inverse_alpha_tau_batch_norm_{a_actual}_{tau_actual}.gif', images, fps=10)
    # if i%499 == 0:
    #     torch.save({"model_parameters" : pinn.state_dict(), "inverse_parameters" : a1.detach().cpu().numpy(), "inverse_parameters_1" : tau1.detach().cpu().numpy()}, f'batch_alpha_tau_inverse_{a_actual}_{tau_actual}.pth')
    
    # if torch.log10(torch.tensor([l[-1]])) < -4.35:
    #     torch.save({"model_parameters" : pinn.state_dict(), "inverse_parameters" : a1.detach().cpu().numpy()}, f'batch_alpha_inverse_{a_actual}.pth')
    #     break

    # if i ==0:
    #   prev_order = torch.round(torch.tensor([math.log10((abs((loss.item()-loss_prev)/loss_prev)))], requires_grad=False))
    # elif torch.round(torch.tensor([math.log10(abs((loss.item()-loss_prev)/loss_prev))], requires_grad=False)) <= prev_order:
    #   count+=1
    # else:
    #   count=1

    # if i>1:
    #     sec=abs((torch.log10(torch.tensor([l[-3]]))-2*torch.log10(torch.tensor([l[-2]]))+torch.log10(torch.tensor([l[-1]])))/2)
    ...
    #     # if count>=patience or torch.log10(torch.tensor([l[-1]])) < torch.tensor([-4.194]):
    #     # if count>=patience or torch.log10(torch.tensor([l[-1]])) < torch.tensor([-4.3]):   for tau_actual=10
    #     if count>=patience or torch.log10(torch.tensor([l[-1]])) < torch.tensor([-4.1956]):
    #         print("Terminating due to Early Stopping")
    #         torch.save({"model_parameters" : pinn.state_dict(), "inverse_parameters" : a1.detach().cpu().numpy()}, f'tau_inverse_{tau_actual}_{a_actual}.pth')
    #         print(f"Training step {i}, Time taken: {iteration_time:.4f}, Total loss : {loss} s, prev_loss: {loss_prev}, cur/prev: {loss.item()/loss_prev}, tau: {tau1}, count: {count}")      
    #         break
